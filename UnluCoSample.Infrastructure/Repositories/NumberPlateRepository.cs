...
namespace UnluCoSample.Infrastructure.Repositories
{
    public class NumberPlateRepository : INumberPlateRepository
    {
        protected readonly UnluCoDbContext _dbContext;
        public NumberPlateRepository(UnluCoDbContext unluCoDbContext)
        {
            _dbContext = unluCoDbContext;
        }
        ...
        {
            return await _dbContext.NumberPlates.FindAsync(id);
        }

        public async Task<NumberPlate> GetByName(string name)
        {
            return await _dbContext.NumberPlates.Where(m => m.Name == name).SingleOrDefaultAsync();
        }
